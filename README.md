...
Работает: nginx + TLS, MCP-гейт, Qdrant, Prometheus + Grafana с дашбордом
по метрикам хоста.

Не сделано:

- **Home Assistant** — ждёт списка устройств. Парк Tuya, заведён в SmartHome
  и оттуда в Яндекс Алису. Local key каждого устройства всё равно берётся
  один раз из облака Tuya. HA встанет параллельно Алисе, не вместо;
  полный уход в локалку Алису сломает — решать до установки.
