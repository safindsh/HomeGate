...
- **Камеры** — в сети шесть Tapo, найдены четыре хоста TP-Link
  (`.61`, `.70`, `.121`, `.140`). RTSP закрыт: у прошивок Tapo с 2022
  года поток отдаётся только после создания **Camera Account** в
  приложении (Настройки → Дополнительно → Учётная запись камеры).
  До этого шага ни HA, ни Frigate, ни `/кадр` в боте камеру не увидят.
  После — `rtsp://логин:пароль@IP:554/stream1` (HD) и `/stream2` (SD)
